...
- [x] 데이터 마이그레이션 — my-prime-jennie DB에서 필요한 데이터 이관 (종목 마스터, 과거 매매 이력 등)
...
### CI/CD 개선 (리뷰 반영)
- [x] deploy.yml: `sleep 10` → healthcheck 기반 대기로 전환 (MariaDB ping + Redis PONG) ✅
- [x] deploy.yml: CI 통과 후에만 배포되도록 연동 (`workflow_run` 또는 `needs`) ✅
- [ ] deploy.yml: .env 복사 시 파일 존재 체크 추가
- [x] deploy.yml: 배포 후 헬스체크 단계 추가 (curl health endpoint) ✅
- [ ] ci.yml: lint job에서 `pip install ruff`만으로 경량화
- [x] ci.yml: `pull_request` 트리거에 development 브랜치 추가 ✅
- [ ] ci.yml: frontend-build에 ESLint 추가 (프로젝트 커질 때)

### 나중에
- [x] 알림 체계 점검 — Telegram 봇 정상 동작 확인 ✅
