...
### 남은 작업 (NICE-TO-HAVE)
- [ ] 데이터 마이그레이션 스크립트 (`scripts/migrate_from_my_prime_jennie.py`)
- [ ] Integration/Contract 테스트 추가 (현재 빈 디렉토리)
- [ ] job-worker 스텁 엔드포인트 실구현 (collect-foreign-holding, collect-dart 등)
- [ ] Telegram collector 서비스 (매크로 Council 브리핑 데이터 소스)
- [ ] Airflow connections 설정 문서 (job_worker, macro_council, price_monitor)

## 다음 할 일 (Next Steps)

- [ ] 프로덕션 배포 전 Docker 빌드 테스트 (`docker compose --profile real build`)
- [ ] Airflow 초기 설정 (DB init, connections 생성)
- [ ] 데이터 마이그레이션 (my-prime-jennie DB → prime-jennie DB)
- [ ] 실거래 테스트 (MOCK 모드)
