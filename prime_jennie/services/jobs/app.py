...
daily-briefing + macro-council 기능 통합 (2026-02-28).
...
from prime_jennie.services.council.pipeline import CouncilInput, CouncilResult, MacroCouncilPipeline
from prime_jennie.services.council.telegram_collector import collect_hedgecat_briefing
...
async def lifespan(app):
    """Startup: Council pipeline + Redis caches 초기화."""
    config = get_config()
    r = redis_lib.Redis.from_url(config.redis.url, decode_responses=True)
    app.state.council_redis = r
...
app = create_app("job-worker", version="1.0.0", lifespan=lifespan, dependencies=["redis", "db"])
...
async def council_trigger(
    ...
    r: redis_lib.Redis = app.state.council_redis
...
@app.get("/jobs/council-insight")
async def council_insight():
    """최신 인사이트 조회."""
...
def _load_global_snapshot(r: redis_lib.Redis, target_date: date) -> GlobalSnapshot | None:
    ...
    for days_ago in range(8):
        ...
        if raw:
            break
    if not raw:
        logger.warning("No macro snapshot found in Redis")
        return None
...
def _load_legacy_insight_as_briefing(r: redis_lib.Redis, target_date: date) -> str:
