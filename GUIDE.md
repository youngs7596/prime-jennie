...
| 역할 | 하는 일 |
|------|--------|
| 🎯 **전략가** (DeepSeek) | 글로벌 경제 지표(환율, 금리, 물가 등), KOSPI 지수, 외국인·기관 자금 흐름, WSJ 뉴스레터를 분석하고 시장 방향을 제시합니다 |
| 🛡️ **리스크 분석가** (DeepSeek) | 전략가의 분석을 검증하고, 정치·지정학적 리스크와 숨겨진 위험 요소를 찾습니다 |
| ⚖️ **수석 심판** (Claude Opus) | 두 전문가의 의견을 종합하여 최종 시장 판정을 내립니다. 가장 고급 AI가 최종 결정 |
...
### 하루에 두 번 + 장중 5분 간격 업데이트

- ⏰ **오전 7:50** — 장 시작 전 전략 수립 (WSJ 뉴스레터 포함)
- ⏰ **오전 11:50** — 장중 상황 재평가
- ⏰ **장중 5분 간격** — Intraday Risk Throttle (9:00~15:55)

> 💡 장중에는 5분마다 KOSPI 등락률과 VIX를 체크하여, 급락 시 자동으로 매수 규모를 축소하고 시장 국면을 하향 조정합니다. 회복 시에는 단계적으로 복원됩니다.
...
| 🔄 **수급** (돈의 흐름) | 20점 | 외국인 순매수(6) + 기관 순매수(8) + 외국인 비율 추세(6) |
...
| 🔄 **갭업 리바운드** | 전일 하락 후 당일 +2% 이상 갭업 + 거래량 1.5배 이상 + 갭 유지 시 매수 |
...
- ✅ 60일 이동평균 대비 **과열 상태**(이격률 초과)인 종목은 매수 차단
...
### 장중 리스크 관리 (Intraday Risk Throttle)
- ✅ **5분마다** KOSPI 등락률과 VIX를 자동 체크
- ✅ KOSPI -1% 이하 또는 VIX 30 이상이면 매수 규모 자동 축소
- ✅ KOSPI -4% 이하면 **매수 완전 차단** (CRITICAL 모드)
- ✅ 회복 시 즉시 복원하지 않고 **단계적으로 복원** (5~30분 지연)
...
### 긴급 강제 청산
- ✅ 텔레그램에서 `/liquidate` 명령으로 특정 종목 **즉시 전량 매도** 가능
- ✅ 실수 방지를 위한 **2단계 안전장치**: 대상 등록(add) → 활성화(arm) 분리
- ✅ emergency stop 상태에서도 강제 청산은 실행됨
...
- ✅ 전체 보유 종목 **최대 10개**
- ✅ 하루 매수 **최대 6건**
- ✅ Council이 회피 권고한 전략 유형의 매수 신호 **자동 차단**
...
- ✅ 60일 이격률이 국면별 임계값 초과 시 매수 차단 (예: 보합장 28% 초과 → 추격매수 방지)
...
매일 오후 5시, 텔레그램으로 **일일 브리핑**이 도착합니다. 또한 아침 7:50에는 WSJ 뉴스레터의 **한국어 요약**이 자동 전송됩니다.
...
| 🌎 **WSJ 요약** | 월스트리트저널 뉴스레터 한국어 요약 (아침 별도 발송) |
...
네, 종목 발굴부터 매수·매도·리포트까지 **100% 자동**입니다. 사용자는 매일 텔레그램 리포트를 확인하고, 필요시 대시보드에서 현황을 점검하면 됩니다. 원하시면 텔레그램 명령어로 긴급 정지(`/stop`), 강제 청산(`/liquidate`), 수동 감시(`/watch`) 등의 제어도 가능합니다.
